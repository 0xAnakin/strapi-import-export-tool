...
async function runExport(cmdTypes, options = {}) {
  ...
  for (const uid of typesToExport) {
    ...
    try {
      ...
      if (strapi.documents) {
        // Strapi v5
        // 1. Get Drafts (includes all content, even unpublished edits)
        const drafts = await strapi.documents(uid).findMany({
            populate,
            status: 'draft' 
        });
        
        // 2. Get Published (to check status)
        ...
        entries = (Array.isArray(drafts) ? drafts : [drafts]).map(item => {
             // If this document is also published, force the publishedAt date from the published version
             // ensuring the Import tool knows to publish it.
             if (publishedMap.has(item.documentId)) {
                 return {
                     ...item,
                     publishedAt: publishedMap.get(item.documentId)
                 };
             }
             // Otherwise it's purely a draft
             return {
                 ...item,
                 publishedAt: null
             };
