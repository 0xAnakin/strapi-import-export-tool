...
async function runImport(inputPath) {
  ...
  // PRE-CLEANUP: Removed to allow Upsert (Update/Overwrite) behavior
  // instead of wiping existing data.
              ...
              if (existing) {
                  ...
                  if (strapi.documents) {
                      await strapi.documents(uid).create({ 
                          data: creationPayload,
                          status: creationPayload.publishedAt ? 'published' : 'draft' 
                      });
                  } else {
  ...
  for (const uid of uids) {
      ...
      for (const item of items) {
          ...
          try {
              if (strapi.documents) {
                  // v5: Use EntityService to update relations. safer than documents().update direct call which might conflict on status/ids
                   await strapi.entityService.update(uid, documentId, { 
                      data: updatePayload
                   });
              } else {
