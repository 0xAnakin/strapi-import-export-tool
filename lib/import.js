      ...
      for (const item of items) {
          ...
          try {
              if (isSingleType) {
                  ...
                  if (existing) {
                      // Use documentId for identification in Strapi v5 if available
                      const identifier = existing.documentId || existing.id;
                      await strapi.entityService.update(uid, identifier, { data: cleanedPayload });
                      console.log(`  Updated entry for ${uid} (ID: ${identifier})`);
                  } else {
