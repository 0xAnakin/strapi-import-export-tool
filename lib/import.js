...
// Helper to copy/overwrite source code
function importSourceCode(importPath, options) {
    ...
    if (fs.existsSync(srcApi)) {
        console.log('--- IMPORTING SOURCE: API ---');
        try {
    ...
    if (fs.existsSync(srcComponents)) {
        console.log('--- IMPORTING SOURCE: COMPONENTS ---');
        try {
...
// Helper to clean source code matches
...
async function runImport(userInputPath, options = {}) {
  ...
  if (options && options.clean) {
      console.log('Cleanup finished. Exiting (clean-only mode).');
